...
def test_delete_pin_message():
    actual_endpoint = f"/api/ai-mentor/orgs/{ORG}/users/{USER_ID}/pin-message/"
    matrix_endpoint = "/api/ai-mentor/orgs/{org}/users/{user_id}/pin-message/"
    ...
    if status == 204:
        summary = "Pinned message deleted successfully"
        notes = "Validated DELETE pin-message endpoint; returned 204 No Content"
        ...
        notes = "Endpoint accessible to both tenant admins and students; verify permissions"

    elif status == 404:
        summary = "Pinned message not found"
        notes = "Check org, user_id, or if a message is currently pinned"
