...
def test_get_recent_messages():
    url = f"{BASE_URL}/api/ai-mentor/orgs/{ORG}/users/{USER_ID}/recent-messages/"
    ...
    print("\n" + "=" * 90)
    print("TEST: Get Recent Messages")
    print("=" * 90)
    ...
    try:
        ...
        if response.status_code == 200:
            print(
                f"| ai-mentor | /api/ai-mentor/orgs/{{org}}/users/{{user_id}}/recent-messages/ | "
                f"GET | ibl.ai docs | 200 | working | "
                f"Recent messages retrieved | Endpoint returned session/message data successfully. |"
            )
