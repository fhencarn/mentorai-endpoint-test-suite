...
def test_list_planned_jobs():
    url = f"{BASE_URL}/api/ai-mentor/orgs/{ORG}/users/{USER_ID}/planned-jobs/"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"| ai-mentor | /api/ai-mentor/orgs/{{org}}/users/{{user_id}}/planned-jobs/ | "
                f"GET | ibl.ai docs | 200 | working | "
                f"Planned jobs list returned (count={count}) | "
                f"Validated planned jobs list endpoint; paginated structure confirmed. |"
            )
