...
def test_delete_clear_chathistory():
    actual_endpoint = f"/api/ai-mentor/orgs/{ORG}/users/{USER_ID}/clear-chathistory"
    matrix_endpoint = "/api/ai-mentor/orgs/{org}/users/{user_id}/clear-chathistory"
    ...
    if status == 204:
        ...
        notes = "Validated DELETE clear chat history endpoint; returned 204 No Content"

    elif status == 400:
        summary = "Bad request"
        notes = f"Check org or user_id; response={clean(data)}"
        ...
        notes = "Current user role may not allow clearing chat history"

    elif status == 404:
        summary = "Clear chat history endpoint not found"
