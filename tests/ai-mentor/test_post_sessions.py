...
def test_post_sessions():
    ...
    endpoint = f"/api/ai-mentor/orgs/{ORG}/users/{USER_ID}/sessions/"
    ...
    try:
        ...
        if status == 200:
            label = "working"
            notes = "Validated session creation endpoint; session_id returned successfully for the provided mentor."
        elif status == 400:
            label = "bad request"
            notes = "Request reached endpoint, but payload may be invalid; verify mentor name formatting."
        elif status == 401:
            label = "unauthorized"
            ...
            label = "not found"
            notes = "Endpoint reachable, but the provided mentor name could not be found for this user/org context."
        else:
            label = "error"
        ...
        print_matrix_row(
            ...
            notes="Script/runtime failure while testing sessions endpoint.",
        )
