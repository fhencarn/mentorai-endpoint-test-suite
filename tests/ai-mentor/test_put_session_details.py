...
def test_put_session_details():
    endpoint_template = "/api/ai-mentor/orgs/{org}/users/{user_id}/sessions/{session_id}/"
    endpoint = f"/api/ai-mentor/orgs/{ORG}/users/{USER_ID}/sessions/{SESSION_ID}/"
    url = f"{BASE_URL}{endpoint}"

    headers = get_auth_headers()

    payload = {
        "title": "Endpoint Test Session Title"
    }
    ...
    try:
        ...
        if status == 200:
            label = "working"
            notes = "Validated session update endpoint; chat session details updated successfully."
        elif status == 400:
            label = "bad request"
            notes = "Request reached endpoint, but payload may be invalid; verify session title format."
        elif status == 401:
            label = "unauthorized"
            ...
            label = "not found"
            notes = "Endpoint reachable, but the provided session_id could not be resolved."
        else:
            label = "error"
        ...
        print_matrix_row(
            ...
            notes="Script/runtime failure while testing sessions update endpoint.",
        )


if __name__ == "__main__":
    test_put_session_details()