...
def test_list_edx_memory():
    endpoint_template = "/api/ai-mentor/orgs/{org}/users/{user_id}/edx-memory/"
    ...
    url = f"{BASE_URL}/api/ai-mentor/orgs/{ORG}/users/{USER_ID}/edx-memory/"
    ...
    print("🔍 Running Test: LIST EDX Memory")
    print(f"URL: {url}")
    ...
    if status_code == 200:
        ...
        if isinstance(results, list) and len(results) > 0:
            ...
            expected_keys = ["id", "username", "student", "course_id", "data", "date_created", "last_modified"]
            sample_fields = [key for key in expected_keys if key in first_item]

        response_summary = f"{count} edx memory record(s) returned"
        notes = f"Validated paginated EDX memory list endpoint; sample fields present: {', '.join(sample_fields) if sample_fields else 'none'}"
