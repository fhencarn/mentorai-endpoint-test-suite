...
def test_list_mentor_categories():
    endpoint_template = "/api/ai-mentor/orgs/{org}/users/{user_id}/mentor/categories/"
    ...
    url = f"{BASE_URL}/api/ai-mentor/orgs/{ORG}/users/{USER_ID}/mentor/categories/"
    ...
    print("🔍 Running Test: LIST Mentor Categories")
    print(f"URL: {url}")
    ...
    if status_code == 200:
        if isinstance(data, list):
            ...
            response_summary = f"{count} mentor categor(y/ies) returned"
            notes = f"Validated mentor categories endpoint; sample fields present: {', '.join(sample_fields) if sample_fields else 'none'}"
        ...
        print_matrix_row(
            ...
            "Expected list of category objects"
        )
        return False
