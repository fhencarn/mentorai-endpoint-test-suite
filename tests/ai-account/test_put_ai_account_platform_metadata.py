...
def test_put_ai_account_platform_metadata():
    ...
    if response.status_code == 200:
        label = "working"
        summary = "Platform metadata updated"
        notes = "Auth successful; updated active_llm and llms list."
    elif response.status_code == 400:
        label = "bad request"
        summary = str(response_data)[:200].replace("\n", " ")
        notes = "Request reached endpoint, but payload may be invalid (check active_llm + llms format)."
        ...
        notes = "Endpoint reached, but platform metadata may not exist yet for this org."
    else:
        label = "error"
        summary = str(response_data)[:200].replace("\n", " ")
    ...
    print("\n--- Endpoint Matrix Row ---")
    print(
        f"| ai-account | {endpoint_template} | PUT | ibl.ai docs | "
        f"{response.status_code} | {label} | {summary} | {notes} |"
    )


if __name__ == "__main__":
    test_put_ai_account_platform_metadata()