...
def test_ai_prompt_metadata():
    endpoint = f"/api/ai-prompt/orgs/{ORG}/users/{USER_ID}/metadata"
    ...
    if status == 200:
        label = "working"
        summary = "Prompt metadata retrieved successfully"
        notes = "Endpoint returned prompt metadata"
    elif status == 400:
        ...
        label = "working"
        summary = "No prompt metadata found for this user"
        notes = "Docs indicate NotFound may occur if no metadata exists"
    elif status >= 500:
        label = "server-error"
