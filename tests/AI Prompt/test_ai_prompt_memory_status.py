...
def test_ai_prompt_memory_status():
    endpoint = f"/api/ai-prompt/orgs/{ORG}/users/{USER_ID}/memory-status/"
    ...
    if status == 200:
        label = "working"
        summary = "Memory status retrieved successfully"
        notes = "Endpoint returned user memory status"
    elif status == 400:
        ...
        label = "working"
        summary = "No memory status found for this user"
