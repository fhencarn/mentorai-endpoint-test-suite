...
def test_ai_prompt_languages():
    endpoint = f"/api/ai-prompt/orgs/{ORG}/users/{USER_ID}/languages/"
    ...
    if status == 200:
        label = "working"
        summary = "Available prompt languages retrieved successfully"
        notes = "Endpoint returned language options for the user"
    elif status == 400:
        ...
        notes = "Check org, user_id, and endpoint path"
    elif status >= 500:
        label = "server-error"
